...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $abonelik_sorgu = "SELECT * FROM doğalgaz_abonelik WHERE email = '$email' AND tc = '$tc' AND ad_soyad = '$ad_soyad' AND telefon = '$telefon'";

    $abonelik_sonuc = $conn->query($abonelik_sorgu);

    if ($abonelik_sonuc->num_rows > 0) {
     
        $iptal_sorgu = "DELETE FROM doğalgaz_abonelik WHERE email = '$email' AND tc = '$tc' AND ad_soyad = '$ad_soyad' AND telefon = '$telefon'";
