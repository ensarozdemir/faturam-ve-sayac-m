...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($tc != $kayit_tc) {
        echo "<div id='sonuc-panel' onclick='hidePanel()' style='display: block; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background-image: linear-gradient(to right top, #add8e6 0%, #1a73e8 100%); border: 1px solid #ced4da; border-radius: 5px; padding: 20px; max-width: 600px;'>";
        echo "<p style='color: white;'>Girdiğiniz TC Kimlik Numarası Size Ait Değil.</p>";
        echo "</div>";
    } else {
        $sql = "SELECT * FROM su_abonelik WHERE tc = '$tc'";
        $result = $conn->query($sql);
