...
if ($result->num_rows > 0) {
	...
	$abonelik_sorgu = "SELECT * FROM doğalgaz_abonelik WHERE email = '$username'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	$abonelik_sorgu = "SELECT * FROM doğalgaz_abonelik WHERE email = '$email'";
    $abonelik_sonuc = $conn->query($abonelik_sorgu);
    if ($abonelik_sonuc->num_rows > 0) {
        echo "<script>
    var message = 'Önceden abone olduğunuz için tekrar abone olamazsınız.';
    alert(message); 
    window.location.href = 'bitirmeProjesi/html/anadoğalgaz.html'; 
</script>";
exit;
    }
   ...
   if ($kayit_ad_soyad !== $ad_soyad || $kayit_tc !== $tc || $kayit_telefon !== $telefon || $kayit_email !== $email) {
    ...
    echo "<script>var message = 'Kayıt bilgileriniz ile abonelik bilgileriniz eşleşmemektedir.';</script>";
        ...
        $sql = "INSERT INTO doğalgaz_abonelik (ad_soyad, tc, telefon, email, mulk, adres, resim_yol, resim_on_yol, resim_arka_yol, abonelik_numarasi, onay, abonelik_durumu) VALUES ('$ad_soyad', '$tc', '$telefon', '$email', '$mulk', '$adres', '$resim_yol', '$resim_on_yol', '$resim_arka_yol', '$abonelik_numarasi', '$onay', 'aktif')";
...
<title>Doğalgaz Abonelik Formu</title>
<link rel="stylesheet" href="bitirmeProjesi/html/css/dAbonelik.css">
<style>
</style>
</head>
<body>
    ...
    <div class="container">
        <h2 class="title">Doğalgaz Abonelik Formu</h2>
        <form action="http://localhost/dAbonelik.php" method="POST" enctype="multipart/form-data" class="form-container">
            ...
            <div class="checkbox-group">
                ...
                <label for="onay">Doğalgaz aboneliği sözleşmesi ve aydınlatma metni</label>
            </div>
    ...
    <div id="aydinlatma-panel" class="aydinlatma-panel">
        <div class="aydinlatma-panel-content">
            ...
            <p>Bu metin doğalgaz aboneliği hakkında detaylı bilgi içerir. Doğalgaz aboneliği sözleşmesi ve aydınlatma metni olarak adlandırılan bu belge, abonelik işlemleri sırasında kullanıcıya sunulan bir belgedir. Bu belgede, doğalgaz tedarikçisi ve tüketici arasındaki hak ve sorumluluklar, fatura ödeme koşulları, aboneliğin iptali ve diğer önemli bilgiler yer alır. Kullanıcıların bu metni dikkatlice okumaları ve anlamaları önemlidir, çünkü doğalgaz aboneliği ile ilgili hak ve yükümlülüklerini belirler. Ayrıca, bu metni onaylamadan önce herhangi bir soruları varsa doğalgaz tedarikçilerine danışmaları önerilir.</p>
